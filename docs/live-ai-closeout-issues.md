...
## Issue 2 — Templates import from `@forkshop/engine`, which doesn't exist in a user install

**Symptom.** The mount page and every board template in
`packages/engine/src/skill/setup.md` import from `@forkshop/engine`:

```tsx
import { ForkshopCanvas, PageTree, responsiveFrameStageDimensions } from "@forkshop/engine"
```

But `@forkshop/engine` is the workspace package (`"private": true` in
`packages/engine/package.json`); it's never published to npm. The CLI's
...
local paths (`copy-files.ts` → `rewriteImports`) — but `@forkshop/engine` is
...
`from "@forkshop/engine"` and there is nothing to resolve it. The dev server
errors with `Module not found: Can't resolve '@forkshop/engine'` on the first
page render.

The reason the playground works: `apps/playground/package.json` has
`"@forkshop/engine": "workspace:*"` and `next.config.mjs` adds
`transpilePackages: ["@forkshop/engine"]` — both monorepo-only mechanisms. A
real user installation has neither.

**Fix sketch.** Either:

- Add `@forkshop/engine` to the CLI's rewriter alias map, pointed at a
  user-side barrel (e.g. `@/components/forkshop`), and have `forkshop init`
  copy `packages/engine/src/index.ts` into the user's project so the barrel
...
`packages/engine/templates/user-claude-md.md` if they show the same pattern.
...
The four prose mentions of `@forkshop/engine` in `user-claude-md.md` (lines
...
`packages/engine/tailwind/forkshop.css` declares an `@font-face` for the
...
1. `manifest-builder.ts` has `fonts: { kind: "asset", items: [] }` — the
   ...
   packages/engine/fonts/"*. The file actually exists today at
   `packages/engine/fonts/raveo/RaveoVF.woff2`, so the precondition is
