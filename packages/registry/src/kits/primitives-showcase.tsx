  ...
  return (
    <div className="flex flex-col gap-16">
      {primitives.map((p) => (
        <div
          key={p.id}
          className="rounded-xl border border-fogma-border bg-fogma-surface p-16"
        >
          <p className="mb-8 font-mono text-xs uppercase tracking-wider text-fogma-fg-tertiary">
            {p.name}
          </p>
          <div>{p.render()}</div>
        </div>
